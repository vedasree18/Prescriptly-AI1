...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (!auth) {
        ...
        const stored = localStorage.getItem('prescriptly_mock_user');
        if (stored) {
            try {
                setCurrentUser(JSON.parse(stored));
            } catch (e) {
                console.error("Failed to parse mock user", e);
                localStorage.removeItem('prescriptly_mock_user');
            }
        }
  ...
  const googleSignIn = async () => {
    if (isMockAuth) {
        ...
        localStorage.setItem('prescriptly_mock_user', JSON.stringify(mockUser));
        return;
    }
